# Technical Specification Document — HalalPilot

| Item | Nilai |
|---|---|
| Versi dokumen | 1.0 (draft untuk build) |
| Tanggal | 2 September 2026 |
| Penulis | Muhammad Fauzan Ramadhan |
| Status | Disetujui untuk Fase A (lokal, 2–5 Sep); revisi 1.1 setelah checklist H1 (6 Sep) |
| Kompetisi | AI HackFest 2026 IDwebhost, kategori Business Automation, subkategori Productivity & Automation, framework OpenClaw, Batch 2 (6–10 Sep 2026) |
| Dokumen terkait | `00-README.md`, `01-schema.sql`, `02-openapi.yaml`, `rules/*.yaml`, `openclaw/*`, `03-demo-script.md`, `04-article-outline.md`, `05-build-plan.md` |
...
| FR-06 | `POST /umk/{id}/evaluate` menghasilkan jalur (`SELF_DECLARE_SIAP`, `SELF_DECLARE_KURANG_DOKUMEN`, `REGULER`, `TIDAK_LAYAK`), skor 0–100, daftar alasan per aturan, daftar dokumen diminta, dan `rules_version`. Keputusan append-only. | M | 3 kasus demo + 6 kasus tepi |
...
- Kamus **wajib dilengkapi** dari lampiran KMA 1360/2021 sebelum H1; struktur file tidak berubah.
...
### 8.1 Referensi
Skema lengkap: `01-schema.sql`. Tambahan untuk implementasi: tabel `idempotency(key, response_json, created_at)` dan kolom `document_req.requested_at`.
...
Kontrak: `02-openapi.yaml`. Konvensi:
...
| Kontrak | `api.mjs` ↔ `02-openapi.yaml` (setiap perintah memetakan ke path/method yang ada) | skrip validasi | 0 perintah tanpa endpoint |
...
| Acceptance demo | Naskah `03-demo-script.md` dijalankan utuh 2× di H4 | rekaman | Tidak ada langkah gagal; durasi 7–9 menit |
...
| E13 | `NIB, PERNYATAAN_HALAL, IKRAR, PENYELIA, DAFTAR_BAHAN, PROSES, FOTO_PRODUK, MANUAL_SJPH` (yang belum ada) |
| E15 | `PERBAIKAN_OSS` |

Dokumen `DAFTAR_BAHAN`, `PROSES`, `MANUAL_SJPH`, `PERNYATAAN_HALAL`, `IKRAR` berstatus `dihasilkan` oleh pembuat dossier dari data yang sudah dikonfirmasi; `NIB`, `FOTO_PRODUK`, `PENYELIA`, `SERT_PEMASOK:*` harus `diterima` dari UMK.

### 17.3 Rujukan
- Kepkaban BPJPH No. 146/2025 (juknis self-declare UMK) — ringkasan bpjph.halal.go.id, Jan 2026.
- KMA No. 1360/2021 (bahan dikecualikan) — lampiran PDF di halal.go.id.
